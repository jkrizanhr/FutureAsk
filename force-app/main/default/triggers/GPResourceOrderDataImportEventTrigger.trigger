trigger GPResourceOrderDataImportEventTrigger on GP_Resource_Order_Data_Import_Event__e(
  after insert
) {
  Integer batchSize = GPDataImportHelper.getBatchSize('GP_Resource_Order_Data_Import_Event__e');
  Integer counter = 0;
  ...
  for (GP_Resource_Order_Data_Import_Event__e event : Trigger.new) {
    ...
    try {
      // Import the data
      new GPResourceOrderDataImportService(event).run();
    } catch (Exception e) {
      ...
      eventError.Origin__c = 'NJE_ResourceOrder';
