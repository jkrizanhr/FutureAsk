trigger GPExpenseDataImportEventTrigger on GP_Expense_Data_Import_Event__e(after insert) {
  Integer batchSize = GPDataImportHelper.getBatchSize('GP_Expense_Data_Import_Event__e');
  Integer counter = 0;
  ...
  for (GP_Expense_Data_Import_Event__e event : Trigger.new) {
    ...
    try {
      // Import the data
      new GPExpenseDataImportService(event).run();
    } catch (Exception e) {
      ...
      eventError.Origin__c = 'NJE_Expenses';
