trigger GPGAUDataImportEventTrigger on GP_GAU_Data_Import_Event__e(after insert) {
  ...
  for (GP_GAU_Data_Import_Event__e event : Trigger.new) {
    ...
    try {
      // Import the data
      new GPGAUDataImportService(event).run();
    } catch (Exception e) {
